...
class AnalysisService:
    ...
    def process_analysis(park: Park, geometry: Any, metadata: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # ============================================================
        # 🔥 DEFINE AS DATAS CONFORME isUpToDate
        # ============================================================

        if is_up_to_date:
            # 🔥 CASO 1 e 4: IGNORA endDate, SEMPRE BUSCA A MAIS RECENTE
            print("📅 Modo: Manter atualizado - buscando imagem mais recente")

            if end_date:
                print(f"⚠️ isUpToDate=true, ignorando endDate={end_date}")
            ...
            # 🔥 CASO 2 e 3: isUpToDate=false
            if end_date:
                # 🔥 CASO 2: Tem endDate → USA PERÍODO
                ...
                # 🔥 CASO 3: Sem endDate → BUSCA A MAIS RECENTE (igual ao caso 1)
                print("📅 Modo: Sem endDate - buscando imagem mais recente")
        ...
        if not is_period:
            ...
            latest_analysis = CoolingAnalysis.query.filter_by(
                ...
                CoolingAnalysis.image_date.like(f"{compare_date}%")
            ).first()
        else:
            # 🔥 BUSCA A MAIS RECENTE DO PERÍODO
            latest_analysis = CoolingAnalysis.query.filter_by(
                park_id=park.id,
                num_buffers=num_buffers,
                buffer_distance=buffer_distance
        ...
        if latest_analysis and (not is_period and latest_analysis.image_date[:10] == compare_date):
        ...
        print(f"⚠️ Calculando nova análise...")
        ...
        result = EarthEngineService.calculate_lst(
            ...
            image_datetime=image_datetime if not is_period else None
        )
